...
/**
 * Sign in and sign up, in one dialog, because they are the same act: prove you
  ...
  return (
    ...
    >
      <div className="p-6 sm:p-8" onClick={(event) => event.stopPropagation()}>
        ...
        {step === 'email' ? (
          <>
            <h2 id="auth-title" className="mt-4 text-center text-xl font-bold sm:text-2xl">
              Sign in to unlock all LobbyHub features
            </h2>
          ...
          <>
            ...
            <p className="mt-2 text-center text-sm text-muted">
              Six digits are on their way to <span className="text-fg">{email}</span>. The code
            ...
            >
              <CodeInput
                value={code}
                // Retyping is the answer to a rejected code, so the boxes stop
                // being red the moment it starts rather than at the next attempt.
                onChange={(value) => {
                  setCode(value)
                  setRejected(false)
                }}
                // The last digit is the whole intent: nobody types six characters
                // and then wants to be asked again by pressing a button.
                onComplete={(value) => {
        ...
        <p className="mt-6 text-center text-xs leading-relaxed text-subtle">
          By continuing you agree that we may store your address to keep you signed in and to
          contact you about servers you claim. Nothing else.
        </p>
      </div>
    </dialog>
  )
