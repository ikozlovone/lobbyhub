...
/**
 * Added to, not replaced by, whatever a caller passes.
 *
 * `className ?? base` was the old rule, and it meant that asking for one extra
 * class silently took the size away with it: `<Icon.refresh className="animate-spin" />`
 * rendered an svg with no width, which a flex row collapses to nothing. The
 * refresh button on the listing looked like it deleted its own icon on click.
 *
 * The size is a default rather than a floor, because a caller that wants a
 * bigger glyph has to be able to say so — two competing `size-*` classes are
 * resolved by stylesheet order, not by the order they appear here, so the
 * default is left out when the caller brings its own.
 */
const SHAPE = 'shrink-0'
const DEFAULT_SIZE = 'size-4'

/** `size-5`, `h-6`, `w-3.5` — anything that would fight with the default. */
const SIZED = /(^|\s)(size|h|w)-/

function iconClass(className?: string): string {
  return [SHAPE, className && SIZED.test(className) ? '' : DEFAULT_SIZE, className]
    .filter(Boolean)
    .join(' ')
}
...
export const Icon = {
  ...
  steam: ({ className }: Props) => (
    <svg aria-hidden viewBox="0 0 24 24" fill="currentColor" className={iconClass(className)}>
  ...
  google: ({ className }: Props) => (
    <svg aria-hidden viewBox="0 0 24 24" className={iconClass(className)}>
      <path
        fill="#4285F4"
